    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="w-full max-w-sm rounded-3xl overflow-hidden border border-devin-border shadow-2xl shadow-black/40">
        ...
        >
          ...
          <p className="text-sm text-devin-teal font-medium tracking-wide mb-1">{contact.title}</p>
          <p className="text-xs text-muted-foreground mb-6">{contact.company}</p>
        ...
        <div className="bg-devin-card px-6">
          <ContactRow icon={Phone} label="Mobile" value={contact.phone} href={`tel:${contact.phoneTel}`} />
          <ContactRow icon={Mail} label="Email" value={contact.email} href={`mailto:${contact.email}`} />
          <ContactRow icon={Building2} label="Empresa" value={contact.company} />
